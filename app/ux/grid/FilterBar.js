...
Ext.define('Ext.ux.grid.FilterBar', {
            ...
            if (column.filter) {
                ...
                if (!column.filter.type) {
                    switch (column.filter.xtype) {
                        case 'combo':
                            column.filter.type = (column.filter.multiSelect ?
                                'list' : 'combo');
                            break;
                        case 'datefield':
                            ...
                            break;
                        case 'numberfield':
                            column.filter.type = (column.filter.allowDecimals ?
                                'float' : 'int');
                            break;
                        default:
    ...
    renderButtons : function () {
        ...
        if (me.showShowHideButton && me.columns.getCount()) {
            ...
            me.showHideEl = Ext.get(Ext.core.DomHelper.append(buttonEl, {
                ...
                'data-qtip' : (me.renderHidden ? me.showHideButtonTooltipDo :
                ...
                switch (operator) {
                    case 'eq':
                        filterFn = function (item) {
                            if (column.filter.type == 'date') {
                                ...
                                return (Ext.isEmpty(item.get(column.dataIndex)) ?
                                    me.autoStoresNullValue :
                                    item.get(column.dataIndex)) == (Ext.isEmpty(newVal) ?
                                    me.autoStoresNullValue :
                                    newVal);
                            }
                        };
                    ...
                    case 'in':
                        filterFn = function (item) {
                            ...
                            return re.test((Ext.isEmpty(item.get(column.dataIndex)) ?
                                me.autoStoresNullValue :
    ...
    applyDelayedFilters : function (field) {
        ...
        me.task.delay(me.updateBuffer, me.applyFilters, me, [
            field
        ]);
    },
    // private
    applyInstantFilters : function (field) {
        ...
        me.task.delay(0, me.applyFilters, me, [
            field
        ]);
    },
    //private
