    ...
    Ext.define('Ext.ux.form.field.ClearButton', {
        ...
        supportsCssTransition : function (el) {
            var styles = [
                ...
                'KhtmlTransitionProperty'
            ];

            var style = el.dom.style;
            for (var i = 0,
                length = styles.length; i < length; ++i) {
        ...
        repositionClearButton : function () {
            ...
            clearButtonEl.alignTo(this.textField.bodyEl, 'tr-tr', [
                -1 * (right + 3),
                5
            ]);
        },
        ...
        updateClearButtonVisibility : function () {
            ...
            if (oldVisible != newVisible) {
                if (this.animateClearButton && this.animateWithCss3) {
                    this.clearButtonEl.removeCls(this.clearButtonCls + (oldVisible ?
                        '-on' :
                        '-off'));
                    clearButtonEl.addCls(this.clearButtonCls + (newVisible ?
                        '-on' : '-off'));
                }
                else {
